...
if (!get_option('bc_d_ie9')) {
    add_option('bc_d_ie9', $sie9);
}
if (!get_option('bc_d_ie10')) {
    add_option('bc_d_ie10', $sie10);
}
if (!get_option('bc_d_ie11')) {
    add_option('bc_d_ie11', $sie11);
}
...
function bcOptions() {
    if ($_POST["submit"]) {
        ...
        if ($_POST["display_nine"] == 'ie9') {
            update_option('bc_d_ie9', 'none');
        } else {
            update_option('bc_d_ie9', 'block');
        }
        if ($_POST["display_ten"] == 'ie10') {
            update_option('bc_d_ie10', 'none');
        } else {
            update_option('bc_d_ie10', 'block');
        }
        if ($_POST["display_elleven"] == 'ie11') {
            update_option('bc_d_ie11', 'none');
        } else {
            update_option('bc_d_ie11', 'block');
        }
    ...
    $ie9_checked = get_option('bc_d_ie9');
    if ($ie9_checked == 'none') {
        $tick9 = 'checked="yes"';
    } else {
        $tick9 = '';
    ...
    $ie10_checked = get_option('bc_d_ie10');
    if ($ie10_checked == 'none') {
        $tick10 = 'checked="yes"';
    } else {
        $tick10 = '';
    }

    $ie11_checked = get_option('bc_d_ie11');
    if ($ie11_checked == 'none') {
        $tick11 = 'checked="yes"';
    } else {
        $tick11 = '';
    }
            ...
            <div style="margin: 20px 0px;">
                ...
                <div class="fifty-right">
                    ...
                    <div>
                     ...
                     <div>
                        <input type="checkbox" name="display_nine" value="ie9" <?php echo $tick9; ?> />IE9
                    </div>
                    <div>
                        <input type="checkbox" name="display_ten" value="ie10" <?php echo $tick10; ?> />IE10
                    </div>
                    <div>
                        <input type="checkbox" name="display_elleven" value="ie11" <?php echo $tick11; ?> />IE11
                    </div>
                    <div>
...
//echo get_option('home').'/wp-content/plugins/get_browser_compatiblity/close.png';
    ...
    $state_9 = get_option('bc_d_ie9');
    $state_10 = get_option('bc_d_ie10');
    $state_11 = get_option('bc_d_ie11');
    ...
    /* For opera */
    if ($is_IE) {
        if (($state_9 == 'block') || ((get_option('bc_swidth') >= $_COOKIE["res_width"]) && (get_option('bc_sheight') > $_COOKIE["res_height"]))) {
            echo '
            <style type="text/css" media="screen">
        ...
        if (($state_10 == 'block') || ((get_option('bc_swidth') >= $_COOKIE["res_width"]) && (get_option('bc_sheight') > $_COOKIE["res_height"]))) {
            echo '
            <style type="text/css" media="screen">
                #chk_brwsr_compat {display: block; background: #f90 url("' . $thePath . 'media/background_image.jpg") no-repeat center top; position: absolute; width: 100% !important; padding: 10px 0px; text-align: center; top: 0; left: 0; color: #' . $text_colour . '; font-family: Arial, Helvetica, sans-serif; font-size: 16px; font-weight: bold; z-index: 99; }
            </style>';
        }
        if (($state_11 == 'block') || ((get_option('bc_swidth') >= $_COOKIE["res_width"]) && (get_option('bc_sheight') > $_COOKIE["res_height"]))) {
            echo '
            <style type="text/css" media="screen">
                #chk_brwsr_compat {display: block; background: #f90 url("' . $thePath . 'media/background_image.jpg") no-repeat center top; position: absolute; width: 100% !important; padding: 10px 0px; text-align: center; top: 0; left: 0; color: #' . $text_colour . '; font-family: Arial, Helvetica, sans-serif; font-size: 16px; font-weight: bold; z-index: 99; }
            </style>';
